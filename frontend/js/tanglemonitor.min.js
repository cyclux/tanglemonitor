...
document.getElementById("hideZero").addEventListener("click",function(){filterForValueTX=!0===checkBoxZero.checked?!0:!1;drawGraph({loop:!1});CalcToplist(!1)},!1);document.getElementById("hideSpecificAddressCheckboxWrapper").addEventListener("click",function(){filterForSpecificAddresses.push(filterAddress);drawGraph({loop:!1});CalcToplist(!1)},!1);var maxTransactionsBuffer=maxTransactions,checkBoxEndless=document.getElementById("endlessMode");
...
timer.length*pxSize*2+offsetHeight+30;)c.height+=50;var b=[];txList=txHistory.find({$and:[filterForValueTX?{value:{$ne:0}}:{},0<filterForSpecificAddresses.length?{address:{$nin:filterForSpecificAddresses}}:{}]});a.loop&&(txList=_.orderBy(txList,["receivedAt"],["asc"]));ctx.font=fontSizeHeader+" "+fontFace;ctx.fillStyle=textColor;ctx.textBaseline="hanging";ctx.textAlign="left";ctx.fillText("Total TX count        "+totalTransactions,margin+10,10);ctx.fillText("Avg. TPS              "+totalTPS,margin+
10,25);ctx.fillText("Avg. conf.ratio       "+totalConfRate+" %",margin+10,40);ctx.fillText("Avg. eff. conf.ratio  "+totalConfRateEff+" %",margin+10,55);ctx.fillText("Avg. conf. time   "+totalConfirmationTime+" min",margin+240,10);ctx.fillText("Avg. CTPS         "+totalCTPS,margin+240,25);ctx.fillText("Avg. MS interval  "+milestoneInterval+" min",margin+240,40);ctx.fillText("Unconfirmed",cWidth-60,10);ctx.fillText("Confirmed",cWidth-60,25);ctx.fillText("Reattached",cWidth+40,10);ctx.fillText("Milestone",
cWidth+40,25);ctx.fillText("Avg.conf.time indicator",cWidth-60+5,40);ctx.fillStyle="rgba(0,0,0,1)";ctx.fillRect(cWidth-75,10,pxSize,pxSize);ctx.fillStyle="rgba(0,255,0,1)";ctx.fillRect(cWidth-75,25,pxSize,pxSize);ctx.fillStyle="rgba(255,255,0,1)";ctx.fillRect(cWidth+25,10,pxSize,pxSize);ctx.fillStyle="rgba(0,0,255,1)";ctx.fillRect(cWidth+25,25,pxSize,pxSize);ctx.fillStyle="rgba(244,65,205,1)";ctx.fillRect(cWidth-75,45,15,3);var d=function(a,d){var e=0,b=2*txPerLine;if(0==d%b){var g=d/b;ctx.font=fontSizeAxis+
" "+fontFace;ctx.fillStyle=textColor;ctx.textBaseline="hanging";ctx.textAlign="right";var f=txList.slice(g*b,g*b+b),b=f.length,f=f.filter(function(a){return!1!==a.confirmed}).length,b=Math.round(f/(f+(b-f))*100),g=Math.round(2*txPerLine/(timer[g+1]/1E3-timer[g]/1E3)*10)/10;ctx.fillText((isNaN(b)?"0":b)+"%"+(isNaN(g)?" [...]":" ["+g.toFixed(1)+" TPS]"),margin-5,a.y+offsetHeight+5)}g=pxColorUnconf;b=strokeColorNorm;if(!1===a.confirmed||void 0===a.confirmed)g=pxColorUnconf,b=strokeColorNorm,g.a=1;"f"===
a.milestone&&!0===a.reattached&&(g=pxColorReattach,b=strokeColorNorm,g.a=1);!0===a.confirmed&&"f"===a.milestone&&(g=pxColorConf,b=strokeColorNorm,g.a=1);"m"===a.milestone&&(ctx.fillStyle="rgba(255, 255, 255, 1)",ctx.fillRect(margin+cWidth+5,a.y+offsetHeight,100,pxSize),ctx.font=fontSizeAxis+" "+fontFace,ctx.fillStyle=textColor,ctx.textBaseline="hanging",ctx.textAlign="left",g=pxColorMilestone,b=strokeColorNorm,f=Math.floor((parseInt(Date.now())-a.receivedAt)/1E3/60),ctx.fillText(f+" min",margin+cWidth+
5,a.y+offsetHeight),g.a=1);"t"===a.milestone&&(g=pxColorMilestone,b=strokeColorNorm,g.a=1);a.address===selectedAddress&&(b=strokeColorSelect,e=1,g.a=1);d===effectiveConfRateIndex&&(ctx.fillStyle="rgba(244,65,205,1)",ctx.fillRect(margin+cWidth+5,a.y+offsetHeight-1,15,3));ctx.fillStyle="rgba("+g.r+","+g.g+","+g.b+","+g.a+")";ctx.fillRect(a.x+margin,a.y+offsetHeight,pxSize,pxSize);ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(a.x+margin,a.y+offsetHeight,pxSize-e,pxSize-e)};txList.map(function(a,h){var e=
calcLineCount(h,pxSize,cWidth),e={x:h*pxSize-e*pxSize*txPerLine,y:e*pxSize,hash:a.hash,bundle:a.bundle,address:a.address,value:a.value,tag:a.tag,confirmed:a.confirmed,reattached:a.reattached,receivedAt:a.receivedAt,ctime:a.ctime,milestone:a.milestone};b.push(e);d(e,h)});pixelMap=b;a.loop&&window.setTimeout(function(){return drawGraph({loop:!0})},tickRate)},CalcToplist=function(a){topListCount+=toplistAdditional;var b=0,d=0,e={},h=[];txList.map(function(a){var b=a.ctime-a.receivedAt;a.confirmed&&h.push(b);
if(e[a.address]){var d=e[a.address].c,g=e[a.address].u;e[a.address].t+=1;a.confirmed?(e[a.address].c=d+1,e[a.address].ct.push(b)):e[a.address].u=g+1}else e[a.address]={c:a.confirmed?1:0,u:a.confirmed?0:1,t:1,ct:a.confirmed?[b]:[]}});e=Object.entries(e);1<toplistMinTX&&(e=_.filter(e,function(a){return a[1].t>=toplistMinTX}));var e=_.orderBy(e,function(a){return a[1].t},[toplistSortIndex[1]]),e=e.slice(0,topListCount),m=[],p=_.mean(h)/1E3/60;e.map(function(a){var e=a[0],g=a[1].u,h=a[1].c;b+=h;d+=g;
var l=a[1].t;a=_.mean(a[1].ct)/1E3/60;var r=a/p*100-100,t=h/l*100,u=g/l*100,q=h/l,v=q/(b/(b+d))*100-100,w=Math.round(l/((Date.now()-txList[0].receivedAt)/1E3)*100)/100,x=Math.round(h/((Date.now()-txList[0].receivedAt)/1E3)*100)/100;m.push([[0],e,[l],[h,t],[g,u],[q],[v],[w],[x],[a],[r]])});topList=m;0<m.length&&createTable(m);a&&window.setTimeout(function(){return CalcToplist(!0)},3E4)},CalcMetricsSummary=function(){var a=Date.now();milestoneMetrics=[];milestoneIntervalList=[];totalTransactions=txList.length;
if(totalTransactions>=maxTransactions){var b=Math.floor(totalTransactions/maxTransactions);txHistory.chain().find(filterForValueTX?{value:{$ne:0}}:{}).limit(txPerLine*b).remove()}var d=[];txList.map(function(a,b){0===b%(2*txPerLine)&&d.push(a.receivedAt)});timer=d;var e=_.meanBy(totalConfirmations,function(a){if(!1===a.milestone)return a.ctime});totalConfirmationTime=_.round(e/1E3/60,1);totalConfirmations=txList.reduce(function(b,d){!0===d.confirmed&&(b.push({ctime:d.ctime-d.receivedAt,milestone:"f"===
d.milestone?!1:!0,effective:d.receivedAt>a-e?!1:!0}),"m"===d.milestone&&milestoneMetrics.push(d.receivedAt));return b},[]);b=_.countBy(totalConfirmations,"effective")["true"];milestoneMetrics.map(function(a,b){0<b&&milestoneIntervalList.push(a-milestoneMetrics[b-1])});milestoneInterval=Math.round(_.mean(milestoneIntervalList)/1E3/60*10)/10;var h=totalConfirmations.length;totalConfRate=Math.round(h/(h+(totalTransactions-h))*1E4)/100;effectiveConfRateIndex=_.findIndex(txList,function(b){return b.receivedAt>
a-e});0<totalTransactions&&(totalTPS=Math.round(totalTransactions/((a-txList[0].receivedAt)/1E3)*100)/100,totalCTPS=Math.round(h/((a-txList[0].receivedAt)/1E3)*100)/100,txList[effectiveConfRateIndex]&&txList[effectiveConfRateIndex].receivedAt&&(totalTPSeff=Math.round(effectiveConfRateIndex/((a-txList[effectiveConfRateIndex].receivedAt)/1E3)*100)/100,totalCTPSeff=Math.round(b/((a-txList[effectiveConfRateIndex].receivedAt)/1E3)*100)/100,totalConfRateEff=Math.round(totalCTPSeff/totalTPSeff*1E4)/100));
20<totalTPS&&!endlessMode&&!manualPoll?maxTransactions=3E4:20>=totalTPS&&!endlessMode&&!manualPoll&&(maxTransactions=15E3);window.setTimeout(function(){return CalcMetricsSummary()},1500)},InitialHistoryPoll=function(a){var b="";"prod"===envState?b="https://tanglemonitor.com:4433/api/v1/getRecentTransactions?amount="+txAmountToPoll:b="http://localhost:8080/api/v1/getRecentTransactions?amount="+txAmountToPoll;fetch(b,{cache:"no-cache"}).then(function(a){return a.json()}).then(function(b){document.getElementById("loading").style.display=
"none";document.getElementById("loadingTX").classList.add("hide");document.getElementById("loadingTX").classList.remove("inline_block");txList=b.txHistory;txHistory&&db.removeCollection("txHistory");txHistory=db.addCollection("txHistory",{unique:["hash"],indices:["address","bundle"]});txHistory.insert(b.txHistory);a&&(drawGraph({loop:!0}),CalcMetricsSummary(),CalcToplist(!0));a&&!websocketActive?InitWebSocket():websocketActive&&console.log("WebSocket already initialized")})["catch"](function(b){console.error("Error fetching txHistory",
b);0<InitialHistoryPollRetries&&(window.setTimeout(function(){return InitialHistoryPoll(a)},2500),InitialHistoryPollRetries--)})},InitWebSocket=function(){if(!websocketActive){websocketActive=!0;var a="";"prod"===envState?a="https://tanglemonitor.com:4434":a="http://localhost:8081";var b=!0;"prod"===envState?b=!0:b=!1;var d=io.connect(a,{secure:b,reconnection:!1});d.on("connect",function(){console.log("Successfully connected to Websocket.. [websocketActive: "+websocketActive+"]");d.on("newTX",function(a){if(![!0].includes(!1)){txList.push(a);
try{txHistory.insert(a)}catch(h){console.log(h)}}});d.on("update",function(a){UpdateTXStatus(a,"txConfirmed")});d.on("updateMilestone",function(a){UpdateTXStatus(a,"Milestone")});d.on("updateReattach",function(a){UpdateTXStatus(a,"Reattach")});d.on("disconnect",function(a){console.log("WebSocket disconnect ["+a+"]");websocketActive=!1;d.close();window.setTimeout(function(){InitWebSocket();console.log("WebSocket reconnecting...")},getRndInteger(100,1E3))});d.on("reconnect",function(a){console.log("WebSocket reconnect ["+
a+"]")});d.on("reconnect_error",function(a){console.log("WebSocket reconnect_error ["+a+"]");websocketActive=!1;window.setTimeout(function(){InitWebSocket()},getRndInteger(10,100))});d.on("connect_timeout",function(a){console.log("WebSocket connect_timeout ["+a+"]");websocketActive=!1;window.setTimeout(function(){InitWebSocket()},getRndInteger(10,100))});d.on("error",function(a){console.log("WebSocket error ["+a+"]")});d.on("connect_error",function(a){console.log("WebSocket connect_error ["+a+"]");
websocketActive=!1;window.setTimeout(function(){InitWebSocket()},getRndInteger(10,100))});window.addEventListener("beforeunload",function(){d.close()})})}},Main=function(){InitialHistoryPoll(!0)};Main();
